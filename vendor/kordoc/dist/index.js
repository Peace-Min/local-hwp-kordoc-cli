...
function renderHtml(markdown, options) {
  ...
  const watermark = options?.watermark ? `<div class="watermark">${escapeHtml(options.watermark)}</div>` : "";
...
async function htmlToPdf(html, options) {
  ...
  const executablePath = process.env.PUPPETEER_EXECUTABLE_PATH ?? findChromiumPath();
  if (!executablePath) {
    throw new KordocError(
  ...
  try {
    ...
    const pdf = await page.pdf({
      ...
      footerTemplate: options?.footer ?? '<div style="font-size:8pt;width:100%;text-align:center;color:#777;"><span class="pageNumber"></span>/<span class="totalPages"></span></div>'
    });
...
async function parsePdf(buffer, options) {
  ...
  try {
    const mod = await import("./parser-NPPJWWAA.js");
